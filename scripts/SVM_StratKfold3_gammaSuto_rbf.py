...
#   University of Iowa
#   Support Vector Machine Classifier (Linear Kernel) Script
##############################################
...
for filepicker in range(0, len(csvFiles)):
    ...
    for iterator in range(1, dataCSV_dummy.shape[1]):
        ...
        for train_index, test_index in loo.split(X,labels):
            ...
            # Defining SVM Linear Kernel
            SVC_Linear = SVC(kernel='linear', probability=True)
            # Model fitting (Training)
            SVC_Linear.fit(train_features, train_labels.ravel())

            # Ready to calculate ROC curve and AUC
            from sklearn.metrics import roc_curve, auc
            # Make predictions based on fitted model
            y_pred_rf = SVC_Linear.predict_proba(test_features)[:, 1]
...
SVM_AUC.to_csv('../../result_auc_for_each_position/SVM_AUC.csv',sep=',',index=False,header=True)
#plt.show()
