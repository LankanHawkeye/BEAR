...
print("Please, type in the top number of features (i.e., selected number of featuers) according to above guidelines.")
...
if int(Join) == min(max_length_list) or int(Join) <= min(max_length_list) and int(Join) > 0:
    ...
    #plt.show()
    # print(mydictionary.get('Relief')[0])

    # plt.figure(1)
    # objects = ('Union', 'At Least 2', 'At Least 3', 'At Least 4', 'SelectedByFive', 'Relief','Symmetric Uncertainty','Gain Ratio','Information Gain',"Correlation","Ensemble")
    # objects = ['Union', 'At Least 2', 'At Least 3', 'At Least 4', 'SelectedByFive', 'Relief', 'Symmetric Uncertainty',
    #           'Gain Ratio', 'Information Gain', 'Correlation', 'Ensemble']

    # y_pos = np.arange(len(objects))
    # print(y_pos)
    # performance = [0.68, 0.67, 0.67, 0.68, 0.69, 0.69, 0.69, 0.66, 0.69, 0.69, 0.69]
    # plt.ylim(0, 1)
    # plt.bar(y_pos, performance, align='center', width=0.4,alpha=0.75,color=['red','plum','steelblue','orange','yellow','green','navy','blue','purple','black','brown'])
    # plt.bar(np.asarray(y_pos), np.asarray(performance), label=objects, align='center', width=0.4, alpha=0.75,
    #        color=['cyan', 'steelblue', 'red', 'orange', 'purple', 'blue', 'black', 'pink', "greenyellow", "green", "navy"])
